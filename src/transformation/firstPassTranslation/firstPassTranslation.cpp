 ...
 *
 **************************************************************/
unsigned int firstPassTranslator(DBTPlateform* platform, unsigned int size, unsigned int sourceStartAddress,
                                 unsigned int sectionStartAddress, unsigned int placeCode)
{
  ...
  int returnedValue = firstPassTranslator_riscv_hw(
      localMipsBinaries, size, platform->vliwInitialConfiguration, sourceStartAddress, sectionStartAddress,
      localVliwBinaries, placeCode, localInsertions, localBlockBoundaries, localUnresolvedJumps_src,
      localUnresolvedJumps_type, localUnresolvedJumps);

  int returnedValueSoft = firstPassTranslator_riscv_sw(
      platform->mipsBinaries, size, platform->vliwInitialConfiguration, sourceStartAddress, sectionStartAddress,
      platform->vliwBinaries, placeCode, platform->insertions, platform->blockBoundaries, platform->unresolvedJumps_src,
  ...
  startProfiler(0);
  int returnedValue = firstPassTranslator_riscv_sw(
      platform->mipsBinaries, size, platform->vliwInitialConfiguration, sourceStartAddress, sectionStartAddress,
      platform->vliwBinaries, placeCode, platform->insertions, platform->blockBoundaries, platform->unresolvedJumps_src,
      platform->unresolvedJumps_type, platform->unresolvedJumps);
  stopProfiler(0);
  ...
  // We copy insertions
  addInsertions((sectionStartAddress - sourceStartAddress) >> 2, placeCode, platform->insertions,
                platform->insertions[0]);
