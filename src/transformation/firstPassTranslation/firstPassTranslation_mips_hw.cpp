...
int generateInterpretationBinaries_loop(uint32 code[1024], uint32 size, uint32 addressStart,
                                        uint128 destinationBinaries[1024], uint32 placeCode, uint32 insertions[256],
                                        uint1 blocksBoundaries[65536], uint32 unresolvedJumps_src[512],
                                        uint32 unresolvedJumps_type[512], int32 unresolvedJumps[512]);
#endif
 ...
 *
 **************************************************************/
uint32 firstPassTranslator_MIPS(DBTPlateform* platform, uint32 size, uint32 codeSectionStart, uint32 addressStart,
                                uint32 placeCode)
{
  ...
  int returnedValue = generateInterpretationBinaries_loop(platform->mipsBinaries, size, addressStart,
                                                          platform->vliwBinaries, placeCode, platform->insertions,
                                                          platform->blockBoundaries, platform->unresolvedJumps_src,
                                                          platform->unresolvedJumps_type, platform->unresolvedJumps);
  ...
  for (int oneUnresolvedJump = 0; oneUnresolvedJump < numberUnresolvedJumps; oneUnresolvedJump++) {
    ...
    unsigned int instructionBeforePreviousDestination =
        readInt(platform->vliwBinaries, 16 * (indexOfDestination - 1) + 12);
                                        ...
                                        uint32 unresolvedJumps_type[512], int32 unresolvedJumps[512])
{
  ...
  while (indexInSourceBinaries < size) {
    ...
    if (enableNextInstruction) {
      ...
      if (op == R && (funct == SLL || funct == SRL || funct == SRA || funct == 0 || funct == SLLV || funct == SRLV ||
                      funct == SRAV)) {
        pred1_reg = rt;
      }
      ...
      if (op == R && funct != JALR) {
        dest_ena = 1;
        dest_reg = rd;
      } else if (!(op == SB || op == SH || op == SW || op == J || op == JAL || op == BEQ || op == BNE || op == BLEZ ||
                   op == BGTZ)) {
        dest_ena = 1;
        dest_reg = rt;
      }
...
int firstPassTranslator(unsigned char* code, unsigned int* size, unsigned int addressStart,
                        unsigned char* destinationBinaries, unsigned int* placeCode, unsigned int& numberInsertions,
                        int* insertions, short* blocksBoundaries, short* proceduresBoundaries)
{
  ...
  while (oneInstructionIndex < *size) {
    ...
    if (enableNextInstruction) {
      ...
      unsigned int oneInstruction = (code[4 * oneInstructionIndex] << 24) + (code[4 * oneInstructionIndex + 1] << 16) +
                                    (code[4 * oneInstructionIndex + 2] << 8) + (code[4 * oneInstructionIndex + 3]);
      ...
      if (op == R) {
        ...
        // First instruction is cmp r32 reg1 reg2
        binaries += (op == REGIMM) ? regimmBindings[rt]
                                   : opcodeBinding[op]; // opcode (here the binding will be done with cmpeq/cmpne
        binaries += 0 << 7;                             // instruction is immediate
        binaries += 0 << 8;                             // instruction do not use br register
        binaries += 32 << 14;                           // Destination
        binaries += pred1_reg << 20;                    // Destination
