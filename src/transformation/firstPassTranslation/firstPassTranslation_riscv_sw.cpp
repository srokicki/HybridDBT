...
int firstPassTranslator_riscv_sw(unsigned int code[1024], unsigned int size, unsigned char conf,
                                 unsigned int addressStart, unsigned int codeSectionStart,
                                 unsigned int destinationBinaries[4 * 1024], unsigned int placeCode,
                                 unsigned int insertions[256], unsigned char blocksBoundaries[65536],
                                 unsigned int unresolvedJumps_src[512], unsigned int unresolvedJumps_type[512],
                                 int unresolvedJumps[512])
{
  ...
  while (indexInSourceBinaries < size || nextInstructionNop || enableNextInstruction) {
    ...
    if (enableNextInstruction) {
      ...
      short imm13 = ((oneInstruction >> 19) & 0x1000) + ((oneInstruction >> 20) & 0x7e0) +
      ...
      if ((rs1 == lastWrittenRegister && lastLatency != 0) || (rs2 == lastWrittenRegister && lastLatency != 0) ||
        ...
        if (funct7 == RISCV_OP_M) {
          ...
          char vexOpcode = (funct3 == RISCV_OP_SLL) ? VEX_SLL : (funct7 == RISCV_OP_SR_SRA) ? VEX_SRA : VEX_SRL;
        ...
        //
        //
        //				int immediateValue = keepHigher ? (imm31_12 >> 13) & 0x7ffff : (imm31_12 >> 12) &
        //0x7ffff; 				char shiftValue = keepHigher ? 13 : 12;
        //
        ...
        //
        //
        //					binaries = assembleRInstruction_sw(functBindingBR_sw[funct3], 32, rs1, rs2);
        ////TODO check order
        //
        ...
        if (funct7 == RISCV_OP_M) {
          ...
          char vexOpcode = (funct3 == RISCV_OPW_SLLW) ? VEX_SLLW : (funct7 == RISCV_OPW_SRW_SRAW) ? VEX_SRAW : VEX_SRLW;
