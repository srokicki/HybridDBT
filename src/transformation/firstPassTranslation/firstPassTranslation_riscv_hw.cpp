	...
	while (indexInSourceBinaries < size || nextInstructionNop || enableNextInstruction){
		...
		if (enableNextInstruction){
			...
			***************************************************************/
			...
			ac_int<1, false> isRdFloat = (funct7 != RISCV_FP_FCVTW) && (funct7 != RISCV_FP_FCMP) && (funct7 != RISCV_FP_FMVXFCLASS);
...
//				}
//				std::cerr << printDecodedInstrRISCV(oneInstruction) << "   " << printDecodedInstr(binaries) << "\n";
...
//				}
				...
				if (funct3 == RISCV_SYSTEM_ENV){
					...
					binaries = assembleIInstruction(VEX_SYSTEM, VEX_ECALL,0);
				}
				else if (funct3 == RISCV_SYSTEM_CSRRS){
					binaries = assembleIInstruction(VEX_SYSTEM, VEX_CSRRS, rd);
					lastWrittenRegister = rd;
