...
int firstPassTranslator_riscv_hw(ac_int<32, false> code[1024], ac_int<32, false> size, ac_int<8, false> conf,
                                 ac_int<32, false> addressStart, ac_int<32, false> codeSectionStart,
                                 ac_int<128, false> destinationBinaries[1024], ac_int<32, false> placeCode,
                                 ac_int<32, false> insertions[256], ac_int<1, false> blocksBoundaries[65536],
                                 ac_int<32, false> unresolvedJumps_src[512],
                                 ac_int<32, false> unresolvedJumps_type[512], ac_int<32, true> unresolvedJumps[512])
{
  ...
  while (indexInSourceBinaries < size || nextInstructionNop || enableNextInstruction) {
    ...
    if (enableNextInstruction) {
      ...
      if ((nextInstruction_rs1 == lastWrittenRegister && lastLatency != 0) ||
          (nextInstruction_rs2 == lastWrittenRegister && lastLatency != 0) ||
          (nextInstruction_rs1 == previousWrittenRegister && previousLatency != 0) ||
          (nextInstruction_rs2 == previousWrittenRegister && previousLatency != 0)) {
        binaries              = 0;
      ...
      if ((rs1 == lastWrittenRegister && lastLatency != 0) || (rs2 == lastWrittenRegister && lastLatency != 0) ||
          (rs1 == previousWrittenRegister && previousLatency != 0) ||
          (rs2 == previousWrittenRegister && previousLatency != 0)) {
        binaries              = 0;
        ...
        //
        //				if (bit31 != bit30){
        //					binaries = assembleIInstruction_sw(VEX_MOVUI, (imm31_12>>14) & 0x7ffff,
        //rd); 					nextInstruction = assembleRiInstruction_sw(VEX_SH2ADDi, rd, rd, (imm31_12>>12)&0x3);
        //
        //					//Mark the insertion
        //					nextInstruction_rd = rd;
        //					nextInstruction_rs1 = rd;
        ...
        //				}
        //				else {
        //					binaries = assembleIInstruction_sw(VEX_MOVUI, (imm31_12>>12) & 0x7ffff,
        //rd);
        //				}
        //				std::cerr << printDecodedInstrRISCV(oneInstruction) << "   " <<
        //printDecodedInstr(binaries) << "\n";
        ...
        //					boundary1 = ((imm13_signed>>2)+indexInSourceBinaries);
        //					setBoundaries2 = 1;
        //					boundary2 = indexInSourceBinaries + 1;//Only plus one because in riscv next
        //instr is not executed
        //
        //
        //					binaries = assembleRInstruction(functBindingBR[funct3], 32, rs1, rs2); //TODO
        //check order
        //
        //					nextInstruction = assembleRiInstruction(VEX_BRF, 32, 0,0);
        //
        //					nextInstruction_stage = 0;
        //					enableNextInstruction = 1;
        //					nextInstruction_rd = 0;
