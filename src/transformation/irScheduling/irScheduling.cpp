                ...
                unsigned char numberFreeRegister, char configuration)
{
  ...
  if (platform->dbtType == DBT_TYPE_HW) {
    ...
    platform->optimizationEnergy += ((int)basicBlockSize) * 4 * 4;
  } else {
    platform->optimizationCycles += basicBlockSize * 635;
    platform->optimizationEnergy += ((int)basicBlockSize) * 635 * 14.48;
  }
  ...
  ac_int<32, false> result = irScheduler_scoreboard_hw(
      opt, basicBlockSize, localBytecode, localVliwBinaries, addressInBinaries, localPlaceOfRegisters,
      numberFreeRegister, localFreeRegisters, issue_width, way_specialisation, localPlaceOfInstr);

  unsigned int swResult = irScheduler_scoreboard_sw(
      opt, basicBlockSize, platform->bytecode, platform->vliwBinaries, addressInBinaries, platform->placeOfRegisters,
  ...
  //	fprintf(stderr, "%d   %x %x %x %x vs %x %x %x %x\n", oneSourceValue, platform->vliwBinaries[4*oneSourceValue+0],
  //platform->vliwBinaries[4*oneSourceValue+1], platform->vliwBinaries[4*oneSourceValue+2],
  //platform->vliwBinaries[4*oneSourceValue+3], 			readInt(localVliwBinaries, 16*oneSourceValue + 0),
  //readInt(localVliwBinaries, 16*oneSourceValue + 4), readInt(localVliwBinaries, 16*oneSourceValue + 8),
  //readInt(localVliwBinaries, 16*oneSourceValue + 12)); 	fprintf(stderr, "\n");
  ...
  startProfiler(2);
  int result = irScheduler_scoreboard_sw(
      opt, basicBlockSize, platform->bytecode, platform->vliwBinaries, addressInBinaries, platform->placeOfRegisters,
      numberFreeRegister, platform->freeRegisters, issue_width, way_specialisation, platform->placeOfInstr);
  stopProfiler(2);
