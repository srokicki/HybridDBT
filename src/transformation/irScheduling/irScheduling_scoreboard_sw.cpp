 ...
 */
unsigned int createInstruction(unsigned int irInstr96, unsigned int irInstr64, unsigned short rIn1, unsigned short rIn2,
                               unsigned short rDest)
{
                                       ...
                                       unsigned int way_specialisation, unsigned int placeOfInstr[256])
{
  ...
  while (instructionId_sw < basicBlockSize) {
    ...
    for (int i = 0; i < 7; ++i) {
      ...
      // WAW
        ...
        if (unitType == 0) {
          ...
          //					ac_int<32, true> test = (typeOfPred == 0) ? (ac_int<32,false>(place+2)) :
          //(ac_int<32,false>(place+1)); 					earliest_place = max(earliest_place, test);
        }
      }
    }
    ...
    char isFPOneReg          = (opCode == VEX_FP && (funct == VEX_FP_FCVTSW || funct == VEX_FP_FCVTSWU ||
                                            funct == VEX_FP_FCVTWS || funct == VEX_FP_FCVTWUS ||
                                            funct == VEX_FP_FMVWX || funct == VEX_FP_FMVXW || funct == VEX_FP_FCLASS));
    char isFPTwoReg          = opCode == VEX_FP && !isFPOneReg;
    char isBranchWithTwoRegs = (opCode == VEX_BR) || (opCode == VEX_BRF) || (opCode == VEX_BGE) ||
    ...
    bool useRin1 = (typeCode == 0 && !isImm) &&
                   (opCode != VEX_FP || (opCode == VEX_FP && funct != VEX_FP_FCVTSW && funct != VEX_FP_FCVTSWU &&
                                         funct != VEX_FP_FCVTWS && funct != VEX_FP_FCVTWUS && funct != VEX_FP_FMVWX &&
                                         funct != VEX_FP_FMVXW && funct != VEX_FP_FCLASS));
    ...
    if (!possible) {
      unsigned int advance =
          (earliest_place > windowPosition_sw + WINDOW_SIZE) ? earliest_place - windowPosition_sw - WINDOW_SIZE + 1 : 1;

      for (int windowOffset = 0; windowOffset < 3; ++windowOffset) {
        char off = offset(windowOffset);

        binaries[(addressInBinaries + incrementInBinaries * (windowPosition_sw + windowOffset)) * 4 + 3] =
            freeSlot_sw[off][0] ? 0 : window_sw[off][0];
        binaries[(addressInBinaries + incrementInBinaries * (windowPosition_sw + windowOffset)) * 4 + 2] =
            freeSlot_sw[off][1] ? 0 : window_sw[off][1];
        binaries[(addressInBinaries + incrementInBinaries * (windowPosition_sw + windowOffset)) * 4 + 1] =
            freeSlot_sw[off][2] ? 0 : window_sw[off][2];
        binaries[(addressInBinaries + incrementInBinaries * (windowPosition_sw + windowOffset)) * 4 + 0] =
            freeSlot_sw[off][3] ? 0 : window_sw[off][3];
        if (issue_width > 4) {
          binaries[(addressInBinaries + incrementInBinaries * (windowPosition_sw + windowOffset)) * 4 + 7] =
              freeSlot_sw[off][4] ? 0 : window_sw[off][4];
          binaries[(addressInBinaries + incrementInBinaries * (windowPosition_sw + windowOffset)) * 4 + 6] =
              freeSlot_sw[off][5] ? 0 : window_sw[off][5];
          binaries[(addressInBinaries + incrementInBinaries * (windowPosition_sw + windowOffset)) * 4 + 5] =
              freeSlot_sw[off][6] ? 0 : window_sw[off][6];
          binaries[(addressInBinaries + incrementInBinaries * (windowPosition_sw + windowOffset)) * 4 + 4] =
              freeSlot_sw[off][7] ? 0 : window_sw[off][7];
        }
    ...
    window_sw[offset(bestWindowOffset)][bestStageId] =
        createInstruction(irInstr96, irInstr64, modifiedRin1, modifiedRin2, modifiedRdest);
  ...
  for (int stageId = 0; stageId < issue_width; stageId++) {
    ...
    if (opcode != 0 &&
        (((spec & 0x2) && (opcode > 0x1f || opcode < VEX_STB)) || (spec & 0x8) ||
         ((spec & 0x1) && (opcode == VEX_BR || opcode == VEX_BRF || opcode == VEX_BGE || opcode == VEX_BLT ||
                           opcode == VEX_BGEU || opcode == VEX_BLTU || opcode == VEX_CALL || opcode == VEX_CALLR ||
                           opcode == VEX_GOTO || opcode == VEX_GOTOR)))) {
