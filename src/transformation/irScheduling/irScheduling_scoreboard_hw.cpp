                                            ...
                                            ac_int<32, false> placeOfInstr[256])
{
  ...
  while (instructionId < basicBlockSize) {
    ...
    ac_int<1, false> isITypeWithDest =
        (opCode == VEX_MOVI || opCode == VEX_CALL || (opCode == VEX_SYSTEM && imm19 == VEX_SYSTEM_CSRRS));
    ...
    if (!possible[0]) {
      ...
      for (ac_int<WINDOW_SIZE_L2 + 1, false> windowOffset = 0; windowOffset < 3; ++windowOffset) {
        ...
        for (int oneBit = 0; oneBit < 4; oneBit++) {
          if (issue_width <= 4 && (mask_spec[oneBit][off] != 0 || maskVal[oneBit] != 0) && !maskVal[127] &&
              !maskVal[126] && !maskVal[125] && !maskVal[124]) {
            maskVal[oneBit]        = (maskVal[oneBit] << 4) + mask_spec[oneBit][off];
            mask_spec[oneBit][off] = 0;
          } else if (issue_width > 4 && (mask_spec[oneBit][off] != 0 || maskVal[oneBit] != 0) && !maskVal[127] &&
                     !maskVal[126] && !maskVal[125] && !maskVal[124]) {
  ...
  for (ac_int<WINDOW_SIZE_L2 + 1, false> windowOffset = 0; windowOffset < WINDOW_SIZE; ++windowOffset) {
    ...
    for (int oneBit = 0; oneBit < 4; oneBit++) {
      if (issue_width <= 4 && (mask_spec[oneBit][off] != 0 || maskVal[oneBit] != 0) && !maskVal[127] && !maskVal[126] &&
          !maskVal[125] && !maskVal[124]) {
        maskVal[oneBit] = (maskVal[oneBit] << 4) + mask_spec[oneBit][off];
      } else if (issue_width > 4 && (mask_spec[oneBit][off] != 0 || maskVal[oneBit] != 0) && !maskVal[127] &&
                 !maskVal[126] && !maskVal[125] && !maskVal[124]) {
        maskVal[oneBit] = (maskVal[oneBit] << 8) + mask_spec[oneBit][off];
  ...
  //firstLoad[0]); 	if (maskVal[1] != 0) 		fprintf(stderr, "for 0 mask is %llx\n", (unsigned long long int) maskVal[1]); 	if
  //(maskVal[2] != 0) 		fprintf(stderr, "for 0 mask is %llx\n", (unsigned long long int) maskVal[2]); 	if (maskVal[3] != 0)
