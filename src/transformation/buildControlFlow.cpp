  ...
  for (int oneInstruction = 0; oneInstruction <= sizeNewlyTranslated; oneInstruction++) {
    ...
    if (blockBoundary && !isInsertion & previousBlockStart < indexInVLIWBinaries) {
      ...
      if (newBlock->vliwEndAddress - 2 * offsetInBinaries == oneJumpSource) {
        ...
        unsigned int destinationInVLIWFromNewMethod = solveUnresolvedJump(
        ...
        if (sectionOfDestination < section) {
          ...
          for (int oneBlockForSucc = 0; oneBlockForSucc < application->numbersBlockInSections[sectionOfDestination];
               oneBlockForSucc++) {
            ...
            if (blockForSucc->sourceStartAddress < newBlock->sourceDestination &&
                blockForSucc->sourceEndAddress > newBlock->sourceDestination) {
  ...
  while (numberBlockToStudy != 0) {
    ...
    for (int oneSection = 0; oneSection < application->numberOfSections; oneSection++) {
      for (int oneBlock = 0; oneBlock < application->numbersBlockInSections[oneSection]; oneBlock++) {
        ...
        unsigned int jumpInstruction = readInt(
            platform->vliwBinaries,
            (application->blocksInSections[oneSection][oneBlock]->vliwEndAddress - 2 * incrementInBinaries) * 16);
        ...
        // We determine the name of successor(s)
        int successor1, successor2, nbSucc;
        if (isConditionalBranch && (application->blocksInSections[oneSection][oneBlock]->sourceDestination ==
                                        currentBlock->sourceStartAddress ||
                                    application->blocksInSections[oneSection][oneBlock]->sourceEndAddress ==
                                        currentBlock->sourceStartAddress)) {
          blocksToStudy[numberBlockToStudy] = application->blocksInSections[oneSection][oneBlock];
          numberBlockToStudy++;
        } else if (isJump) {
          if (application->blocksInSections[oneSection][oneBlock]->sourceDestination ==
              currentBlock->sourceStartAddress) {
            blocksToStudy[numberBlockToStudy] = application->blocksInSections[oneSection][oneBlock];
          ...
          if (application->blocksInSections[oneSection][oneBlock]->sourceEndAddress ==
              currentBlock->sourceStartAddress) {
            blocksToStudy[numberBlockToStudy] = application->blocksInSections[oneSection][oneBlock];
    ...
    *(block->reference) = NULL;
    ...
    if (block->nbInstr == 0) {
      ...
      for (int oneBytecodeInstr = 0; oneBytecodeInstr < blockSize; oneBytecodeInstr++) {
        ...
        printBytecodeInstruction(oneBytecodeInstr, readInt(platform->bytecode, oneBytecodeInstr * 16 + 0),
                                 readInt(platform->bytecode, oneBytecodeInstr * 16 + 4),
                                 readInt(platform->bytecode, oneBytecodeInstr * 16 + 8),
                                 readInt(platform->bytecode, oneBytecodeInstr * 16 + 12));
      }
