...
IRProcedure* rescheduleProcedure_schedule(DBTPlateform* platform, IRProcedure* procedure, int writePlace)
{
  ...
  for (int oneBlock = 0; oneBlock < procedure->nbBlock; oneBlock++) {
    ...
    if (1) {
      for (int i = procedure->blocks[oneBlock]->vliwStartAddress; i < procedure->blocks[oneBlock]->vliwEndAddress;
           i++) {
  ...
  for (int oneBlock = 0; oneBlock < procedure->nbBlock; oneBlock++) {
      ...
      for (int i = 0; i < block->nbInstr; i++)
        Log::printf(LOG_SCHEDULE_PROC, "%s ",
                    printBytecodeInstruction(
                        i, readInt(platform->bytecode, i * 16 + 0), readInt(platform->bytecode, i * 16 + 4),
...
#ifdef IR_SUCC
      result->blocks[oneBlock]->jumpPlaces[oneJump] =
          ((int)platform->placeOfInstr[block->jumpIds[oneJump]]) + writePlace;
#else
      result->blocks[oneBlock]->jumpPlaces[oneJump] =
          incrementInBinaries * ((int)platform->placeOfInstr[block->jumpIds[oneJump]]) + writePlace;
#endif
    }
    ...
    if ((isReturnBlock || isCallBlock) &&
        readInt(platform->vliwBinaries,
                16 * result->blocks[oneBlock]->jumpPlaces[result->blocks[oneBlock]->nbJumps - 1] +
                    16 * incrementInBinaries) != 0) {

      // We need room for the reconf instruction, we add two lines at the end. TODO: this should be done only when
      // reconf is activated
      writeInt(platform->vliwBinaries,
               16 * result->blocks[oneBlock]->jumpPlaces[result->blocks[oneBlock]->nbJumps - 1] +
                   16 * 2 * incrementInBinaries,
               readInt(platform->vliwBinaries,
                       16 * result->blocks[oneBlock]->jumpPlaces[result->blocks[oneBlock]->nbJumps - 1]));
      writeInt(platform->vliwBinaries, 16 * result->blocks[oneBlock]->jumpPlaces[result->blocks[oneBlock]->nbJumps - 1],
               0);
      ...
      if (readInt(platform->vliwBinaries, 16 * result->blocks[oneBlock]->vliwEndAddress - 16 * incrementInBinaries) !=
          0) {
    ...
    // Indirect jumps are weird : the block is marked as having not succ and no jump (consequently we cannot use
    // jumpPlace)
...
int rescheduleProcedure_commit(DBTPlateform* platform, IRProcedure* procedure, int writePlace,
                               IRProcedure* scheduledProc)
{
  ...
  for (int oneBlock = 0; oneBlock < procedure->nbBlock; oneBlock++) {
    ...
    for (int oneJump = 0; oneJump < block->nbJumps; oneJump++) {
      ...
      if (jumpOpcode == VEX_BR || jumpOpcode == VEX_BRF || jumpOpcode == VEX_BGE || jumpOpcode == VEX_BLT ||
        ...
        writeInt(platform->vliwBinaries, 16 * block->jumpPlaces[oneJump],
                 (oldJump & 0xfff0007f) | ((offset & 0x1fff) << 7));
      } else if (jumpOpcode == VEX_GOTO) {
        if (block->nbSucc > oneJump) {
          int dest             = block->successors[oneJump]->vliwStartAddress;
          unsigned int oldJump = readInt(platform->vliwBinaries, 16 * block->jumpPlaces[oneJump]);
          writeInt(platform->vliwBinaries, 16 * block->jumpPlaces[oneJump],
                   (oldJump & 0xfc00007f) | ((dest & 0x7ffff) << 7));
        }
  ...
  for (int oneBlock = 0; oneBlock < procedure->nbBlock; oneBlock++) {
    ...
    if (block->vliwEndAddress - block->vliwStartAddress > 2) {
      ...
      if (getIssueWidth(procedure->previousConfiguration) <= 4) {
        ...
        writeInt(platform->vliwBinaries, 16 * originalEntry + 0,
                 assembleIInstruction_sw(VEX_GOTO, block->vliwStartAddress, 0));
        ...
        writeInt(platform->vliwBinaries, 16 * originalEntry + 16,
                 getReconfigurationInstruction(procedure->configuration));
        ...
        for (int oneSpec = 0; oneSpec < 4; oneSpec++) {
          if (block->specAddr[oneSpec] != 0) {
            writeInt(platform->vliwBinaries, 16 * originalEntry + 4,
                     assembleMemoryInstruction_sw(VEX_SPEC_INIT, 0, 0, block->specAddr[oneSpec], 1, oneSpec));
        ...
        writeInt(platform->vliwBinaries, 16 * originalEntry + 24, 0);
        writeInt(platform->vliwBinaries, 16 * originalEntry + 28, 0);
        writeInt(platform->vliwBinaries, 16 * originalEntry + 32 + 0,
                 getReconfigurationInstruction(procedure->configuration));
      ...
      if (getIssueWidth(platform->vliwInitialConfiguration) > 4) {
        if (platform->vexSimulator->PC == 4 * block->oldVliwStartAddress ||
            platform->vexSimulator->PC == 4 * (block->oldVliwStartAddress + 2))
          platform->vexSimulator->doStep(2);

        writeInt(platform->vliwBinaries, 16 * block->oldVliwStartAddress + 0,
                 assembleIInstruction_sw(VEX_GOTO, block->vliwStartAddress, 0));
        writeInt(platform->vliwBinaries, 16 * block->oldVliwStartAddress + 32,
                 getReconfigurationInstruction(procedure->configuration));

      } else {
        if (platform->vexSimulator->PC == 4 * block->oldVliwStartAddress ||
            platform->vexSimulator->PC == 4 * (block->oldVliwStartAddress + 1))
          platform->vexSimulator->doStep(2);

        writeInt(platform->vliwBinaries, 16 * block->oldVliwStartAddress + 0,
                 assembleIInstruction_sw(VEX_GOTO, block->vliwStartAddress, 0));
        writeInt(platform->vliwBinaries, 16 * block->oldVliwStartAddress + 16,
                 getReconfigurationInstruction(procedure->configuration));
      }
    }
    ...
    if (isReturnBlock) {
      ...
      if (block->nbJumps == 1) {
        if (readInt(platform->vliwBinaries, 16 * block->jumpPlaces[block->nbJumps - 1] + 16 * incrementInBinaries) == 0)
          writeInt(platform->vliwBinaries, 16 * block->jumpPlaces[block->nbJumps - 1] + 16 * incrementInBinaries,
                   getReconfigurationInstruction(platform->vliwInitialConfiguration));
        else {
        ...
        if (readInt(platform->vliwBinaries, 16 * block->vliwEndAddress - 16 * incrementInBinaries) == 0)
          writeInt(platform->vliwBinaries, 16 * block->vliwEndAddress - 16 * incrementInBinaries,
                   getReconfigurationInstruction(platform->vliwInitialConfiguration));
        else {
          Log::printf(LOG_ERROR, "Failing when inserting reconfs at the end of a procedure...\nExiting...");
          exit(-1);
    ...
    if (isCallBlock) {
      ...
      writeInt(platform->vliwBinaries, 16 * block->vliwEndAddress - offsetFirstLine,
               getReconfigurationInstruction(platform->vliwInitialConfiguration));
...
void inPlaceBlockReschedule(IRBlock* block, DBTPlateform* platform, int writePlace)
{
  ...
  if (isCurrentlyInBlock) {
    ...
    if (instructionInEnd == 0) {
      ...
      Log::printf(LOG_ERROR, "In optimize basic block, execution is in the middle of a block and programm cannot stop "
                             "it...\n exiting...\n");
      exit(-1);
    }
  }
  ...
  // TODO WARNING We do not handle in place rescheduling when the configuration have been modified. We will have to
  // handle that.
  ...
  if (platform->debugLevel > 1 || 1) {
    ...
    for (int i = 0; i < block->nbInstr; i++)
      Log::printf(
          LOG_SCHEDULE_PROC, "%s ",
          printBytecodeInstruction(i, readInt(platform->bytecode, i * 16 + 0), readInt(platform->bytecode, i * 16 + 4),
                                   readInt(platform->bytecode, i * 16 + 8), readInt(platform->bytecode, i * 16 + 12))
              .c_str());
  }
  ...
  if (block->vliwStartAddress + binaSize <= block->vliwEndAddress) {
    ...
    memcpy(&platform->vliwBinaries[4 * block->vliwStartAddress], &platform->vliwBinaries[4 * writePlace],
           (binaSize)*4 * sizeof(unsigned int));
    ...
    for (int oneJump = 0; oneJump < block->nbJumps; oneJump++) {
      ...
      block->jumpPlaces[oneJump] =
          incrementInBinaries * ((int)platform->placeOfInstr[block->jumpIds[oneJump]]) + block->vliwStartAddress;
#endif
    }
    ...
    for (int oneSpec = 0; oneSpec < 4; oneSpec++) {
      if (block->specAddr[oneSpec] != 0) {
        Log::fprintf(LOG_SCHEDULE_PROC, stderr, "Mask is %llx %llx\n",
                     (unsigned long long int)maskVal[oneSpec].slc<64>(64),
                     (unsigned long long int)maskVal[oneSpec].slc<64>(0));
        platform->specInfo[8 * block->specAddr[oneSpec] + 2] = readInt(maskVal, oneSpec * 16 + 12);
        platform->specInfo[8 * block->specAddr[oneSpec] + 3] = readInt(maskVal, oneSpec * 16 + 8);
        platform->specInfo[8 * block->specAddr[oneSpec] + 4] = readInt(maskVal, oneSpec * 16 + 4);
        platform->specInfo[8 * block->specAddr[oneSpec] + 5] = readInt(maskVal, oneSpec * 16);
    ...
    if (block->nbSucc == block->nbJumps + 1 &&
        block->vliwStartAddress + binaSize + 1 * incrementInBinaries < block->vliwEndAddress) {
