...
/****************************************************************************************************************
 * Function getReadWriteRegisters will analyze the block and build the list of registers that are read in the block and
 *of the register written in the block.
 *
 * Arguments:
...
IRBlock* unrollLoops(IRBlock* block, bool ignoreRegs, short* outputRegsToIgnore, char nbIgnore)
{
   ...
   * -> The merging where the entry blocks has two successors. In this case, the execution may leave the block before
   *its end. For this reason we have to write into new register which will be allocated by the scheduler. We then use
   *the conditional instruction to ensure that we do modify the behavior of the program.
   ...
   * 	  	-> If isEscapable we transform memory stores into conditional memory stores, we say writing instruction
   *to allocate a new register instead of using the normal one.
   * 	  	-> For each operand of a given instruction, if the operand is global (>256) then we check if the said
   *register has not been modified by the block 1. If so we change the reference by a non global one and we add a data
   *dependency. Otherwise, we just change the local reference by adding an offset to the operand (the offset will be the
   *size of block 1).
   * 	  	-> For each dependencies of a given instruction, we add an offset in order to refer to the correct
   *instruction index in the newly created block. Once again, this offset will be the size of block 1.
   * 	  	-> The first memory operation found will depend on the last memory operation found on block 1 to ensure
   *the memory coherence
   *
   ...
   *write from the block 1 (to ensure that there is no WAR dependency violated).
   *
  ...
  // useSetc says that we use conditional instruction for solving speculation. Other alternative is the use of correctly
  // handled alloc
  ...
  for (int oneSuccessor = 0; oneSuccessor < block->nbSucc; oneSuccessor++)
    Log::printf(LOG_SCHEDULE_PROC, " (instr %d, dest %d) ",
                oneSuccessor < block->nbJumps ? block->jumpIds[oneSuccessor] : -1,
                block->successors[oneSuccessor]->sourceStartAddress);
  ...
  for (int i = 0; i < block->nbInstr; i++) {
    Log::printf(
        LOG_SCHEDULE_PROC, "%s ",
        printBytecodeInstruction(i, readInt(block->instructions, i * 16 + 0), readInt(block->instructions, i * 16 + 4),
                                 readInt(block->instructions, i * 16 + 8), readInt(block->instructions, i * 16 + 12))
            .c_str());
  }
  Log::printf(LOG_SCHEDULE_PROC, "\n");
  ...
  for (int oneInstr = 0; oneInstr < sizeofEntryBlock; oneInstr++) {
    ...
    if (opcode == VEX_BR || opcode == VEX_BRF || opcode == VEX_BGE || opcode == VEX_BLT || opcode == VEX_BGEU ||
        opcode == VEX_BLTU) {
  ...
  for (int oneUnroll = 1; oneUnroll < unrollingFactor; oneUnroll++) {
    ...
    for (int oneInstr = 0; oneInstr < block->nbInstr; oneInstr++) {
      ...
      // -> The operand is greater than 256 then it is an access to global register and thus we need to check if there
      // is a dep to add
      for (int oneOperand = 0; oneOperand < nbOperand; oneOperand++) {
      ...
      if (shiftedOpcode == (VEX_STD >> 3)) {
        ...
        // In any case, we have to ensure a dependency from last mem instruction from previous block (this is only
        // needed for first memory store)
      ...
      if (opcode == VEX_GOTO || opcode == VEX_GOTOR || opcode == VEX_BRF || opcode == VEX_BR || opcode == VEX_BGE ||
          opcode == VEX_BLT || opcode == VEX_BGEU || opcode == VEX_BLTU || opcode == VEX_CALL || opcode == VEX_CALLR) {
    ...
    if (isEscape) {
      ...
      for (int oneReg = 1; oneReg < 64; oneReg++) {
        if (lastWriteReg[oneReg] >= 0) {
          if (outputRegsToIgnore[oneReg]) {
            ...
            // We add a control dependency from one of the last four cond instruction (note: this array is initialized
            // with four times the setcond instruction)
      ...
      for (int oneReg = 0; oneReg < 128; oneReg++) {
        if (outputRegsToIgnore[oneReg]) {
          ...
          write128(result->instructions, result->nbInstr * 16,
                   assembleRBytecodeInstruction(2, 0, VEX_ADD, lastWriteReg[oneReg], 256, 256 + oneReg, 0));
  ...
  for (int oneSuccessor = 0; oneSuccessor < result->nbSucc; oneSuccessor++)
    Log::printf(LOG_SCHEDULE_PROC, " (instr %d, dest %d) ",
                oneSuccessor < result->nbJumps ? result->jumpIds[oneSuccessor] : -1,
                result->successors[oneSuccessor]->sourceStartAddress);
                    ...
                    char nbIgnore)
{
  ...
  bool isDropEscape =
      isEscape && useSetc && secondBlock->nbSucc == secondBlock->nbJumps + 1 &&
      entryBlock->successors[entryBlock->nbSucc - 2] == secondBlock->successors[secondBlock->nbSucc - 1];
  ...
  for (int oneSuccessor = 0; oneSuccessor < entryBlock->nbSucc; oneSuccessor++)
    Log::printf(LOG_SCHEDULE_PROC, " (instr %d, dest %d) ",
                oneSuccessor < entryBlock->nbJumps ? entryBlock->jumpIds[oneSuccessor] : -1,
                entryBlock->successors[oneSuccessor]->sourceStartAddress);
  ...
  for (int oneSuccessor = 0; oneSuccessor < secondBlock->nbSucc; oneSuccessor++)
    Log::printf(LOG_SCHEDULE_PROC, " (instr %d, dest %d) ",
                oneSuccessor < secondBlock->nbJumps ? secondBlock->jumpIds[oneSuccessor] : -1,
                secondBlock->successors[oneSuccessor]->sourceStartAddress);

  Log::printf(LOG_SCHEDULE_PROC, "\n");

  for (int i = 0; i < entryBlock->nbInstr; i++) {
    Log::printf(LOG_SCHEDULE_PROC, "%s ",
                printBytecodeInstruction(
                    i, readInt(entryBlock->instructions, i * 16 + 0), readInt(entryBlock->instructions, i * 16 + 4),
                    readInt(entryBlock->instructions, i * 16 + 8), readInt(entryBlock->instructions, i * 16 + 12))
                    .c_str());
  }
  Log::printf(LOG_SCHEDULE_PROC, "\n");
  ...
  for (int i = 0; i < secondBlock->nbInstr; i++) {
    Log::printf(LOG_SCHEDULE_PROC, "%s ",
                printBytecodeInstruction(
                    i, readInt(secondBlock->instructions, i * 16 + 0), readInt(secondBlock->instructions, i * 16 + 4),
                    readInt(secondBlock->instructions, i * 16 + 8), readInt(secondBlock->instructions, i * 16 + 12))
                    .c_str());
  }
  ...
  for (int oneInstr = 0; oneInstr < secondBlock->nbInstr; oneInstr++) {
    ...
    // For each operand register, there are two possibilities:
    // -> The operand is lower than 256, then it is a reference to another instruction from the block: we correct it
    // -> The operand is greater than 256 then it is an access to global register and thus we need to check if there is
    // a dep to add
    for (int oneOperand = 0; oneOperand < nbOperand; oneOperand++) {
    ...
    if (shiftedOpcode == (VEX_STD >> 3)) {
      ...
      // In any case, we have to ensure a dependency from last mem instruction from previous block (this is only needed
      // for first memory store)
      for (int oneLastMemInstr = 0; oneLastMemInstr < nbLastMemInstr; oneLastMemInstr++)
...
//			break;
    ...
    if (entryBlock == secondBlock) {
      ...
      result->nbSucc++;
               ...
               (entryBlock->successors[oneJump] != secondBlock ||
                (entryBlock->successors[oneJump] == secondBlock && entryBlock == secondBlock))) {
      result->addJump(entryBlock->jumpIds[oneJump], -1);
  ...
  for (int i = 0; i < result->nbInstr; i++) {
    Log::printf(LOG_SCHEDULE_PROC, "%s ",
                printBytecodeInstruction(
                    i, readInt(result->instructions, i * 16 + 0), readInt(result->instructions, i * 16 + 4),
                    readInt(result->instructions, i * 16 + 8), readInt(result->instructions, i * 16 + 12))
                    .c_str());
  }
...
void buildTraces(DBTPlateform* platform, IRProcedure* procedure, int optLevel)
{
      ...
      if (block->nbJumps + 1 == block->nbSucc && block->nbSucc == 2 && block->successor1 == block) {
        ...
        if (block->nbInstr < 100 && nbBlocksToAdd < 10) {
          ...
          for (int i = 0; i < block->nbInstr; i++) {
            Log::printf(LOG_SCHEDULE_PROC, "%s ",
                        printBytecodeInstruction(
                            i, readInt(block->instructions, i * 16 + 0), readInt(block->instructions, i * 16 + 4),
                            readInt(block->instructions, i * 16 + 8), readInt(block->instructions, i * 16 + 12))
                            .c_str());
          }
          ...
          if (nbIgnoredRegs > 0) {
            ...
            //							Log::printf(0, "%s ", printBytecodeInstruction(i, readInt(newBlock->instructions, i*16+0),
            //readInt(newBlock->instructions, i*16+4), readInt(newBlock->instructions, i*16+8),
            //readInt(newBlock->instructions, i*16+12)).c_str());
            //						}
          }
  ...
  for (int oneBlock = 0; oneBlock < procedure->nbBlock; oneBlock++) {
    ...
    if (blockToAddId < nbBlocksToAdd &&
        blocksToAdd[blockToAddId]->sourceStartAddress < procedure->blocks[oneBlock]->sourceStartAddress) {
