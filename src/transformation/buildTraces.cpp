...
void buildTraces(DBTPlateform* platform, IRProcedure* procedure, int optLevel)
{
  ...
  for (unsigned int oneBlock = 0; oneBlock < procedure->nbBlock; oneBlock++) {
    if (procedure->blocks[oneBlock]->nbInstr != 0) {
      ...
      if (block->nbJumps + 1 == block->nbSucc && block->nbSucc == 2 &&
          block->successors[0] == block->sourceStartAddress) {
        ...
        if (block->nbInstr < 100 && nbBlocksToAdd < 10) {
          ...
          Log::logScheduleProc << "******************** " << (unsigned int)block->sourceStartAddress << "  ---  "
                               << (unsigned int)block->sourceEndAddress << " *********************\n";
          Log::logScheduleProc << "******************************************************************\n";
  ...
  if (optLevel >= 4) {
    ...
    for (unsigned int oneBlock = 0; oneBlock < procedure->nbBlock; oneBlock++) {
      ...
      if (block->blockState == IRBLOCK_PERFECT_LOOP || block->blockState == IRBLOCK_UNROLLED ||
          block->blockState == IRBLOCK_TRACE || block->nbInstr < 8)
        continue;
