...
unsigned int insertCodeForInsertions(DBTPlateform* platform, int start, unsigned int startAddress)
{
  ...
  //	//		| init = startAddress| stw r4 -4(sp)		|					| r33 = r33 >>
  ...
  //	writeInt(platform->vliwBinaries, cycle*16+12, 0);
  //
  //	//		| 				 | 					| r33 = r33 & -4096	| init = r33 &
  //0xfff 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries, cycle*16+4,
  //0); 	writeInt(platform->vliwBinaries, cycle*16+8, assembleRiInstruction(VEX_ANDi, 33, 33, -4096));
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleRiInstruction(VEX_ANDi, 4, 33, 0xfff));
  //
  //	//		| init -= startAddr	 | stw r6 -12(sp)		|					| offset =
  //7 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0 /*assembleRiInstruction(VEX_SUBi, 4, 4, startAddress
  //& 0xfff)*/); 	writeInt(platform->vliwBinaries, cycle*16+4, assembleRiInstruction(VEX_STD, 6, 2, -24));
  //	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleIInstruction(VEX_MOVI, 7, 5));
  //
  //	//		|init = init>>2		 | stw r7 -16(sp)		|					| offset =
  //offset<<24 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, assembleRiInstruction(VEX_SRLi, 4, 4, 2));
  ...
  //
  //	//		| offset offset + r33<<1| stw r8 -20(sp)		|					| start =
  //MAXNB*2 	cycle+=offset; 	char operation = (MAX_INSERTION_PER_SECTION == 2048) ? VEX_SH3ADD :
  //(MAX_INSERTION_PER_SECTION == 1024) ? VEX_SH2ADD : (MAX_INSERTION_PER_SECTION == 512) ? VEX_SH1ADD :  VEX_ADD;
  ...
  //
  //	// bcl: | 				     | r33 = ldw 8(v1)     	|	start = 0				| init = init +
  ...
  //
  //	// 		| 					 |						| 					| t1 = cmple r33
  //init 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries, cycle*16+4,
  //0); 	writeInt(platform->vliwBinaries, cycle*16+8, 0); 	writeInt(platform->vliwBinaries, cycle*16+12,
  //assembleRInstruction(VEX_CMPLE, 9, 33, 4));
  //
  //	// 		|					 |						| v1 = t1 * size	| init = init -
  //size 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries, cycle*16+4,
  //0); 	writeInt(platform->vliwBinaries, cycle*16+8, assembleRInstruction(VEX_MPY, 6, 9, 8));
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleRInstruction(VEX_SUB, 4, 4, 8));
  //
  //	// 		| t1 = cmpeqi size 1 |						|					| size = size
  //>>1 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, assembleRiInstruction(VEX_CMPNEi, 9, 8, 1));
  //	writeInt(platform->vliwBinaries, cycle*16+4, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12,  assembleRiInstruction(VEX_SRLi, 8, 8, 1));
  //
  //	// 		| 				 | start += v1			| init = init + v1	| v1 = offset +
  //size<<2 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries, cycle*16+4,
  //assembleRInstruction(VEX_ADD, 7, 7, 6)); 	writeInt(platform->vliwBinaries, cycle*16+8, assembleRInstruction(VEX_ADD,
  //4, 4, 6)); 	writeInt(platform->vliwBinaries, cycle*16+12, assembleRInstruction(VEX_SH2ADD, 6, 8, 5));
  //
  //	// 		| 				 | 					 	|
  //| 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries, cycle*16+4, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, 0);
  //
  //	// 		| br t1	 | 					| 					| v1 = (start<<2) +
  //v1 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, assembleIInstruction(VEX_BR, (bcl-cycle)<<2, 9));
  //	writeInt(platform->vliwBinaries, cycle*16+4, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  ...
  //
  //	//		|					 | ldw v1 4(offset)		|
  //| 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries, cycle*16+4,
  //assembleRiInstruction(VEX_LDW, 6, 5, 4)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, 0);
  //
  //	cycle+=offset;
  //	writeInt(platform->vliwBinaries, cycle*16+0, 0);
  ...
  //| 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries, cycle*16+4,
  //assembleRiInstruction(VEX_LDD, 4, 2, -8)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleRInstruction(VEX_ADD, 8, 4, 6));
  //
  //	//		|					 | ldw r5 -8(sp)		|					| r8
  //++ 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries, cycle*16+4,
  //assembleRiInstruction(VEX_LDD, 5, 2, -16)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleRiInstruction(VEX_ADDi, 8, 8, 1));
  //
  //	//		|					 | ldw r6 -12(sp)		|
  //| 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries, cycle*16+4,
  //assembleRiInstruction(VEX_LDD, 6, 2, -24)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, 0);
  //
  //	//		| 					| ldw r7 -16(sp)		|					| r8 =
  //r8<<2 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries, cycle*16+4,
  //assembleRiInstruction(VEX_LDD, 7, 2, -32)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  ...
  //| 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries, cycle*16+4,
  //assembleRiInstruction(VEX_LDD, 9, 2, -48)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, 0);
  //
  //	//		|					 | ldw r8 -20(sp)		|
