...
/* Version 1.1 : TODO
 * Will add support for multiple code areas. The idea is to start with an allocation table. Considering the address of
 * the destination, we will be able to find the start point of an array containing insertion on the corresponding
 * section of code. This section will be dimensioned with 1024 instructions. This allocation table will also contain the
 * translated offset of the instruction which will be a starting value to compute the new destination.
 *
                   ...
                   unsigned int* insertionsToInsert, unsigned int numberInsertions)
{
  ...
  int offset  = section << (SHIFT_FOR_INSERTION_SECTION -
                           2); // globalSection * size = globalSection * 16 * (4+4) = globalSection * 0x80
  ...
  for (unsigned int oneInsertion = 0; oneInsertion < numberInsertions; oneInsertion++) {
    storeWordFromInsertionMemory(offset + oneInsertion + 2,
                                 insertionsToInsert[1 + oneInsertion] - blockStartAddressInVLIW);
  }
...
int solveUnresolvedJump(DBTPlateform* platform, unsigned int initialDestination)
{
  ...
  unsigned int offset  = section << (SHIFT_FOR_INSERTION_SECTION -
                                    2); // globalSection * size = globalSection * 16 * (4+4) = globalSection * 0x80
  ...
  while (size != 1) {
    ...
    // loadWordFromInsertionMemory(offset + 2 + start + size/2));
    size = size / 2;
 ...
 *
 *  Function returns the sum of the start value and the number of instructions added (ie. the place where the rest of
 *binaries may be written)
 ********************************************************************************************/
...
unsigned int insertCodeForInsertions(DBTPlateform* platform, int start, unsigned int startAddress)
{
   ...
   *
   * The search algorithm correspond to the search algorithm describe in function solveUnresolvedJump but written in IR
   * format. The first block initialize every thing and set the init value correctly. The loop will look in the middle
   * of the possible insertion limit and see if we are greater or smaller. Depending on this, it will increase the
   * begining or the end of the search path. Once the correct number of dependencies is found, the correct destination
   * is computed and we jump there.
   *
  ...
  write128(platform->bytecode, nbInstr * 16,
           assembleIBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_MOVI, 256 + 37, 0x8, 0)); // 0
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleRiBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SLLi, 0, 28, 256 + 37, 0)); // 1
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleRiBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_ANDi, 256 + 33, 0x70, 256 + 34, 0)); // 2
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SUB, 2, 1, 256 + 34, 0)); // 3
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_LDD, 3, -8, false, 0, 256 + 35, 0)); // 4
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_LDD, 3, -16, false, 0, 256 + 36, 0)); // 5
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_STD, 3, -8, false, 0, 256 + 33, 0)); // 6
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleRiBytecodeInstruction(STAGE_CODE_CONTROL, 0, VEX_BRF, 4, increment * 3, 256 + 33, 0)); // 7
  nbInstr++;
  ...
  write128(platform->bytecode, nbInstr * 16,
           assembleIBytecodeInstruction(STAGE_CODE_CONTROL, 0, VEX_GOTOR, 256 + 36, 0, 0)); // 0
  nbInstr++;
  ...
  write128(platform->bytecode, nbInstr * 16,
           assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_STD, 256 + 2, -8, false, 0, 256 + offset_start,
                                             0)); // 0
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_STD, 256 + 2, -16, false, 0, 256 + init_start,
                                             0)); // 1
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_STD, 256 + 2, -24, false, 0, 256 + value, 0)); // 2
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_STD, 256 + 2, -32, false, 0, 256 + size, 0)); // 3
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_STD, 256 + 2, -40, false, 0, 256 + tmp1, 0)); // 4
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_STD, 256 + 2, -48, false, 0, 256 + tmp2, 0)); // 5
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_STD, 256 + 2, -56, false, 0, 256 + test1, 0)); // 6
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_STD, 256 + 2, -64, false, 0, 256 + test2, 0)); // 7
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleIBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_MOVI, 256 + offset_start, startAddress, 0)); // 8
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SUB, 8, 256 + 33, 256 + 33, 0)); // 9
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleRiBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SRAi, 9, 12, 256 + offset_start, 0)); // 10
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleRiBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SLLi, 10, SHIFT_FOR_INSERTION_SECTION,
                                         256 + offset_start, 0)); // 11
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleIBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_MOVI, 256 + size, 0x8, 0)); // 12
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleRiBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SLLi, 12, 28, 256 + size, 0)); // 13
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_ADD, 13, 11, 256 + offset_start, 0)); // 14
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleRiBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_ANDi, 9, 4095, 256 + init_start, 0)); // 15
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_LDW, 14, 4, false, 0, 256 + 33, 0)); // 16
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleIBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_MOVI, 256 + size, MAX_INSERTION_PER_SECTION / 2, 0)); // 17
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_ADD, 256 + 0, 14, 256 + value, 0)); // 18
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SH2ADD, 18, 17, 256 + value, 0)); // 19
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleRiBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SRAi, 15, 2, 256 + init_start, 0)); // 20
  nbInstr++;
  ...
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_LDW, 256 + value, 8, false, 0, 256 + value, 0)); // 0
  nbInstr++;

  write128(
      platform->bytecode, nbInstr * 16,
      assembleRiBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SRAi, 0, increment == 2 ? 1 : 0, 256 + value, 0)); // 1
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SH2ADD, 256 + offset_start, 256 + size, 256 + tmp1,
                                        0)); // 2
  nbInstr++;

  write128(
      platform->bytecode, nbInstr * 16,
      assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_ADD, 256 + init_start, 256 + size, 256 + tmp2, 0)); // 3
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_CMPLE, 3, 1, 256 + test1, 0)); // 4
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_CMPNE, 256 + size, 256 + 0, 256 + test2, 0)); // 5
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleRiBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SRAi, 256 + size, 1, 256 + size, 0)); // 6
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SETc, 4, 2, 256 + offset_start, 0)); // 7
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SETc, 4, 3, 256 + init_start, 0)); // 8
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SH2ADD, 7, 6, 256 + value, 0)); // 9
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleIBytecodeInstruction(STAGE_CODE_CONTROL, 0, VEX_BR, 5, 0, 0)); // 11
  nbInstr++;
  ...
  writeInt(platform->vliwBinaries, (start - 2 * increment) * 16,
           assembleRiInstruction_sw(VEX_BRF, test2, 0, (-(binaSize - 2 * increment))));
  ...
  if (platform->vliwInitialIssueWidth > 4)
    write128(
        platform->bytecode, nbInstr * 16,
        assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SH1ADD, 256 + 33, 256 + init_start, 256 + 33, 0)); // 0
  else
    write128(platform->bytecode, nbInstr * 16,
             assembleRBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_ADD, 256 + 33, 256 + init_start, 256 + 33, 0)); // 0

  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleRiBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_SLLi, 0, 2, 256 + 33, 0)); // 1
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_LDD, 256 + 2, -8, false, 0, 256 + offset_start,
                                             0)); // 2
  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_LDD, 256 + 2, -16, false, 0, 256 + init_start,
                                             0)); // 3
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_LDD, 256 + 2, -24, false, 0, 256 + value, 0)); // 4
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_LDD, 256 + 2, -32, false, 0, 256 + size, 0)); // 5
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_LDD, 256 + 2, -40, false, 0, 256 + tmp1, 0)); // 6
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_LDD, 256 + 2, -48, false, 0, 256 + tmp2, 0)); // 7
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_LDD, 256 + 2, -56, false, 0, 256 + test1, 0)); // 8
  nbInstr++;
  write128(
      platform->bytecode, nbInstr * 16,
      assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_LDD, 256 + 2, -64, false, 0, 256 + test2, 0)); // 9

  nbInstr++;
  write128(platform->bytecode, nbInstr * 16,
           assembleRiBytecodeInstruction(STAGE_CODE_ARITH, 0, VEX_ADDi, 256 + 33, 4 * increment, 256 + 33, 0)); // 10
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleMemoryBytecodeInstruction(STAGE_CODE_MEMORY, 0, VEX_STD, 256 + 34, -16, false, 0, 1, 0)); // 11
  nbInstr++;

  write128(platform->bytecode, nbInstr * 16,
           assembleIBytecodeInstruction(STAGE_CODE_CONTROL, 0, VEX_GOTOR, 1, 0, 0)); // 12
  nbInstr++;
  ...
  printf("Result %d\n", start);
  for (int oneInstr = 0; oneInstr < start; oneInstr++) {
    std::cout << oneInstr << " " << printDecodedInstr(platform->vliwBinaries[4 * oneInstr + 3]) << " "
              << printDecodedInstr(platform->vliwBinaries[4 * oneInstr + 2]) << " "
              << printDecodedInstr(platform->vliwBinaries[4 * oneInstr + 1]) << " "
              << printDecodedInstr(platform->vliwBinaries[4 * oneInstr]) << "\n";
  }
  ...
  //	//		| init = startAddress| stw r4 -4(sp)		|					| r33 =
  // r33
  //>>
  // 2 	int cycle = start; 	writeInt(platform->vliwBinaries, cycle*16+0, assembleIInstruction(VEX_MOVI, startAddress
  // & 0x7ffff, 4)); 	writeInt(platform->vliwBinaries, cycle*16+4, assembleRiInstruction(VEX_STD, 4, 2, -8));
  ...
  //
  //	//		| 				 | 					| r33 = r33 & -4096
  //| init = r33 & 0xfff 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0);
  // writeInt(platform->vliwBinaries,
  // cycle*16+4, 0); 	writeInt(platform->vliwBinaries, cycle*16+8, assembleRiInstruction(VEX_ANDi, 33, 33, -4096));
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleRiInstruction(VEX_ANDi, 4, 33, 0xfff));
  //
  //	//		| init -= startAddr	 | stw r6 -12(sp)		| | offset = 7 	cycle+=offset;
  // writeInt(platform->vliwBinaries, cycle*16+0, 0 /*assembleRiInstruction(VEX_SUBi, 4, 4,
  // startAddress & 0xfff)*/); 	writeInt(platform->vliwBinaries, cycle*16+4, assembleRiInstruction(VEX_STD, 6, 2, -24));
  //	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleIInstruction(VEX_MOVI, 7, 5));
  //
  //	//		|init = init>>2		 | stw r7 -16(sp)		| | offset
  //=
  // offset<<24 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, assembleRiInstruction(VEX_SRLi, 4,
  // 4, 2)); 	writeInt(platform->vliwBinaries, cycle*16+4, assembleRiInstruction(VEX_STD, 7, 2, -32));
  //	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleRiInstruction(VEX_SLLi, 5, 5, 24));
  //
  //	//		| offset offset + r33<<1| stw r8 -20(sp)		| | start = MAXNB*2 	cycle+=offset;
  // char operation = (MAX_INSERTION_PER_SECTION == 2048) ? VEX_SH3ADD : (MAX_INSERTION_PER_SECTION == 1024) ?
  // VEX_SH2ADD
  //: (MAX_INSERTION_PER_SECTION == 512) ? VEX_SH1ADD :  VEX_ADD; 	writeInt(platform->vliwBinaries, cycle*16+0,
  // assembleRInstruction(VEX_SH1ADD, 5, 33, 5)); 	writeInt(platform->vliwBinaries, cycle*16+4,
  // assembleRiInstruction(VEX_STD, 8, 2, -40)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleIInstruction(VEX_MOVI,2*MAX_INSERTION_PER_SECTION, 7));
  //
  //	//		| v1 = offset + start  | stw r9 -24(sp)		|					| size =
  // 256 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, assembleRInstruction(VEX_ADD, 6, 5, 7));
  ...
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleRiInstruction(VEX_ADDi, 8, 0,
  // MAX_INSERTION_PER_SECTION/2));
  //
  //	// bcl: | 				     | r33 = ldw 8(v1)     	|	start = 0
  //| init = init
  //+ size 	cycle+=offset; 	int bcl = cycle; 	writeInt(platform->vliwBinaries, cycle*16+0, 0);
  ...
  //
  //	// 		| 					 |						|
  //| t1 = cmple r33 init 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0);
  // writeInt(platform->vliwBinaries,
  // cycle*16+4, 0); 	writeInt(platform->vliwBinaries, cycle*16+8, 0); 	writeInt(platform->vliwBinaries,
  // cycle*16+12, assembleRInstruction(VEX_CMPLE, 9, 33, 4));
  //
  //	// 		|					 |						| v1 =
  // t1
  //* size | init = init
  //- size 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries,
  // cycle*16+4, 0); 	writeInt(platform->vliwBinaries, cycle*16+8, assembleRInstruction(VEX_MPY, 6, 9, 8));
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleRInstruction(VEX_SUB, 4, 4, 8));
  //
  //	// 		| t1 = cmpeqi size 1 |						|
  //| size = size
  //>>1 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, assembleRiInstruction(VEX_CMPNEi, 9, 8,
  // 1)); 	writeInt(platform->vliwBinaries, cycle*16+4, 0); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12,  assembleRiInstruction(VEX_SRLi, 8, 8, 1));
  //
  //	// 		| 				 | start += v1			| init = init + v1	| v1 =
  // offset
  //+ size<<2 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries,
  // cycle*16+4, assembleRInstruction(VEX_ADD, 7, 7, 6)); 	writeInt(platform->vliwBinaries, cycle*16+8,
  // assembleRInstruction(VEX_ADD, 4, 4, 6)); 	writeInt(platform->vliwBinaries, cycle*16+12,
  // assembleRInstruction(VEX_SH2ADD, 6, 8, 5));
  //
  //	// 		| 				 | 					 	|
  //| 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries,
  // cycle*16+4, 0); 	writeInt(platform->vliwBinaries, cycle*16+8, 0); 	writeInt(platform->vliwBinaries,
  // cycle*16+12, 0);
  //
  //	// 		| br t1	 | 					| 					| v1 =
  //(start<<2)
  //+
  // v1 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, assembleIInstruction(VEX_BR,
  // (bcl-cycle)<<2, 9)); 	writeInt(platform->vliwBinaries, cycle*16+4, 0); 	writeInt(platform->vliwBinaries,
  // cycle*16+8, 0);
  ...
  //	//		|					 | ldw v1 4(offset)		|
  //| 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries,
  // cycle*16+4, assembleRiInstruction(VEX_LDW, 6, 5, 4)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, 0);
  //
  //	cycle+=offset;
  //	writeInt(platform->vliwBinaries, cycle*16+0, 0);
  ...
  //| 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries,
  // cycle*16+4, assembleRiInstruction(VEX_LDD, 4, 2, -8)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleRInstruction(VEX_ADD, 8, 4, 6));
  //
  //	//		|					 | ldw r5 -8(sp)		|
  //| r8
  //++ 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries,
  // cycle*16+4, assembleRiInstruction(VEX_LDD, 5, 2, -16)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleRiInstruction(VEX_ADDi, 8, 8, 1));
  //
  //	//		|					 | ldw r6 -12(sp)		|
  //| 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries,
  // cycle*16+4, assembleRiInstruction(VEX_LDD, 6, 2, -24)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, 0);
  //
  //	//		| 					| ldw r7 -16(sp)		|
  //| r8 = r8<<2 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0);
  // writeInt(platform->vliwBinaries,
  // cycle*16+4, assembleRiInstruction(VEX_LDD, 7, 2, -32)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, assembleRiInstruction(VEX_SLLi, 8, 8,
  // platform->vliwInitialIssueWidth>4 ? 3 : 2));
  //
  //	//		| gotor r8			 | ldw r9 -24(sp)		|
  //| 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, 0); 	writeInt(platform->vliwBinaries,
  // cycle*16+4, assembleRiInstruction(VEX_LDD, 9, 2, -48)); 	writeInt(platform->vliwBinaries, cycle*16+8, 0);
  //	writeInt(platform->vliwBinaries, cycle*16+12, 0);
  //
  //	//		|					 | ldw r8 -20(sp)		|
  //| 	cycle+=offset; 	writeInt(platform->vliwBinaries, cycle*16+0, assembleIInstruction(VEX_GOTOR, 0, 8));
