...
int main(int argc, char* argv[])
{
  ...
  static std::map<std::string, FILE*> ios = {{"stdin", stdin}, {"stdout", stdout}, {"stderr", stderr}};
  ...
  if (cfg.has("h") || binaryFile == NULL) {
    ...
    Log::logError << "where elfFile is an RISCV64-imf elf file, args are the argument given to the simulated "
                     "application and options are one of the following:\n";
    ...
    Log::logError << "\t-o file\t\tAdding an output file to use as standard output for simulated application. Can add "
                     "up to 10 files by repeating -o file\n";
  ...
  if (size > MEMORY_SIZE) {
    Log::logError << "ERROR: Size of source binaries is " << size
                  << ". Current implementation only accept size lower then " << MEMORY_SIZE << "\n";
    exit(-1);
  }
  ...
  while (runStatus == 0) {
      ...
      for (int oneProcedure = 0; oneProcedure < application.numberProcedures; oneProcedure++) {
        ...
        if (procedure->state == 0) {
          ...
          if (procedure->configuration != oldConf || procedure->configurationScores[procedure->configuration] == 0) {
            IRProcedure* scheduledProcedure =
                rescheduleProcedure_schedule(&dbtPlateform, &application, procedure, placeCode);
            ...
            if (score > procedure->configurationScores[procedure->previousConfiguration]) {
              placeCode =
                  rescheduleProcedure_commit(&dbtPlateform, &application, procedure, placeCode, scheduledProcedure);
            } else {
    ...
    for (int oneBlock = 0; oneBlock < profiler.getNumberProfiledBlocks(); oneBlock++) {
      ...
      if ((MAX_PROC_COUNT == -1 || dbtPlateform.procedureOptCounter < MAX_PROC_COUNT) && block != NULL &&
          OPTLEVEL >= 2 && profileResult >= 15 &&
          (block->blockState == IRBLOCK_STATE_SCHEDULED || block->blockState == IRBLOCK_STATE_PROFILED)) {
        ...
        if (!errorCode) {
          ...
          placeCode = rescheduleProcedure(&dbtPlateform, &application,
    ...
    for (auto& block : application) {
      if (block.sourceStartAddress != -1) {
        if ((MAX_SCHEDULE_COUNT == -1 || dbtPlateform.blockScheduleCounter < MAX_SCHEDULE_COUNT) && OPTLEVEL >= 1 &&
            (block.sourceEndAddress - block.sourceStartAddress > 4 ||
             (block.sourceDestination != -1 && block.sourceDestination <= block.sourceStartAddress)) &&
  ...
  dbtPlateform.vexSimulator->timeInConfig[dbtPlateform.vexSimulator->currentConfig] +=
      (dbtPlateform.vexSimulator->cycle - dbtPlateform.vexSimulator->lastReconf);
