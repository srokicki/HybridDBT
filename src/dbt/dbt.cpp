...
int main(int argc, char* argv[])
{
  ...
  if (cfg.has("h") || binaryFile == NULL) {
    ...
    Log::printf(0, "where elfFile is an RISCV64-imf elf file, args are the argument given to the simulated application "
    ...
    Log::printf(0, "\t-o file\t\tAdding an output file to use as standard output for simulated application. Can add up "
                   "to 10 files by repeating -o file\n");
  ...
  for (int oneUnresolvedJump = 0; oneUnresolvedJump < unresolvedJumpsArray[0]; oneUnresolvedJump++) {
    ...
    if (destinationInVLIWFromNewMethod == -1) {
      Log::printf(LOG_ERROR, "A jump from %d to %x is still unresolved... (%d insertions)\n", source,
                  initialDestination, insertionsArray[(initialDestination >> 10) << 11]);
      exit(-1);
    } else {
      int immediateValue =
    ...
    for (int oneBlock = 0; oneBlock < profiler.getNumberProfiledBlocks(); oneBlock++) {
      ...
      if ((MAX_PROC_COUNT == -1 || dbtPlateform.procedureOptCounter < MAX_PROC_COUNT) && block != NULL &&
        ...
        if (!errorCode) {
          ...
          placeCode =
              rescheduleProcedure(&dbtPlateform, application.procedures[application.numberProcedures - 1], placeCode);
          dbtPlateform.procedureOptCounter++;
      ...
      for (int oneBlock = 0; oneBlock < application.numbersBlockInSections[oneSection]; oneBlock++) {
        ...
        if (block != NULL && block->sourceStartAddress != -1) {
          if ((MAX_SCHEDULE_COUNT == -1 || dbtPlateform.blockScheduleCounter < MAX_SCHEDULE_COUNT) &&
              OPTLEVEL >= 1 /* && block->sourceEndAddress - block->sourceStartAddress > profileGap */ &&
