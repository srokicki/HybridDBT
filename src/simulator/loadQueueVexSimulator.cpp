...
LoadQueueVexSimulator::LoadQueueVexSimulator(unsigned int* instructionMemory, unsigned int* specData)
    : VexSimulator(instructionMemory)
{
  this->speculationData = specData;
  this->PC              = 0;
};
...
void LoadQueueVexSimulator::doMem(ExtoMem extoMem, MemtoWB* memtoWB)
{
  ...
  if (((extoMem.opCode >> 4) == 0x1 || extoMem.opCode == VEX_FLW || extoMem.opCode == VEX_FSW) && extoMem.isSpec) {
    ...
    partitionnedLoadQueue(extoMem.pc, extoMem.result, extoMem.funct, clear, &rollback, this->speculationData, init,
                          extoMem.result, &mask, &rollbackPoint);
    ...
    if (rollback && !extoMem.isRollback && (extoMem.opCode >> 3) != (VEX_LDD >> 3) && extoMem.opCode != VEX_FLW) {
      ...
      // int) mask, (unsigned long long int) rollbackPoint); 			fprintf(stderr, "endrollback is %lld\n",
...
// Redirect instructions to thier own ways
  ...
  ftoDC6.instruction = (this->unitActivation[5] & (this->mask[58] | !this->rollback))
                           ? (this->muxValues[0] ? instructions[1] : instructions[5])
                           : nopInstr;
  ftoDC7.instruction = (this->unitActivation[6] & (this->mask[57] | !this->rollback))
                           ? (this->muxValues[1] ? instructions[2] : instructions[6])
                           : nopInstr;
  ftoDC8.instruction = (this->unitActivation[7] & (this->mask[56] | !this->rollback))
                           ? (this->muxValues[2] ? instructions[3] : instructions[7])
                           : nopInstr;
  ...
  if (this->rollback) {
    ...
    } else if ((this->issueWidth <= 4 && NEXT_PC != PC + 4) || (this->issueWidth > 4 && NEXT_PC != PC + 8) ||
               PC == endRollback) {
      rollback = 0;
      mask     = 0;
      PC       = NEXT_PC;
  ...
  if (debugLevel >= 1) {
    ...
    for (int oneRegister = 0; oneRegister < 38; oneRegister++) {
      fprintf(stderr, "%lx;", (long)regFile.readRegister(oneRegister));
    }
    fprintf(stderr, ";;%lx;", (long)regFile.readRegister(63));
