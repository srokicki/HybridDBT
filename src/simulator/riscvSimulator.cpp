...
void RiscvSimulator::doStep()
{
  ...
  switch (opcode) {
    ...
    case RISCV_SYSTEM:
      ...
      // For modelling pipeline performances
      if (17 == this->lastWrittenRegister || 10 == this->lastWrittenRegister || 11 == this->lastWrittenRegister ||
          12 == this->lastWrittenRegister || 13 == this->lastWrittenRegister) {
        if (this->lastIsLoad)
          this->cycle += LOSS_PIPELINE_HAZARD_FORWARDED;
        else
          this->cycle += LOSS_PIPELINE_HAZARD;
    ...
    case RISCV_FMADD:
      ...
      if (rs1 + 64 == this->lastWrittenRegister || rs2 + 64 == this->lastWrittenRegister ||
          rs3 + 64 == this->lastWrittenRegister) {
        if (this->lastIsLoad)
          this->cycle += LOSS_PIPELINE_HAZARD_FORWARDED;
        else
          this->cycle += LOSS_PIPELINE_HAZARD;
      ...
      switch (funct7) {
        ...
        default:
          printf("In FP part of switch opcode, instr %x is not handled yet(%x)  pC is %x\n", (int)ins,
                 this->heapAddress, (int)this->cycle);
          break;
      }
    ...
    default:
      ...
      //ins, this->heapAddress, this->cycle);
      break;
  }
  REG[0] = 0;
  n_inst = n_inst + 1;
