...
void VexSimulator::doWB(struct MemtoWB memtoWB)
{
  ...
  if (memtoWB.isFloat && memtoWB.WBena) {
    ...
    regFile.writeRegister(memtoWB.dest, memtoWB.result, true);
  }
}
#ifdef __CATAPULT
...
void VexSimulator::doDC(struct FtoDC ftoDC, struct DCtoEx* dctoEx)
{
  ...
  ac_int<64, true> regValueA = regFile.readRegister(RA);
  ac_int<64, true> regValueB = regFile.readRegister(secondRegAccess);
...
void VexSimulator::doDCBr(struct FtoDC ftoDC, struct DCtoEx* dctoEx)
{
  ...
  ac_int<64, false> regValueA_u = 0;
  ac_int<64, false> regValueB_u = 0;
  ...
  switch (OP) { // Select the right operation and place the right values into the right operands
    ...
    case VEX_SYSTEM:
      ...
      if ((IMM19_s & 0xf) == VEX_SYSTEM_ECALL) {
        dctoEx->dataa = this->solveSyscall(regFile.readRegister(17), regFile.readRegister(10), regFile.readRegister(11),
                                           regFile.readRegister(12), regFile.readRegister(13));
        dctoEx->dest  = 10;
        dctoEx->opCode = VEX_MOVI;
...
int VexSimulator::initializeRun(int mainPc, int argc, char* argv[])
{
  ...
  PC = mainPc;
  regFile.writeRegister(2, 0x70000, false);
  stop = 0;
...
int VexSimulator::doStep(unsigned int numberCycles)
{

  this->stop                       = 0;
  unsigned int initialCycleCounter = cycle;
