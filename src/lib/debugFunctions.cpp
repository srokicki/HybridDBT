...
void debugFirstPassResult(DBTPlateform platform, int start, int end, int sourceBinariesStartAddress){
	...
	for (int oneVLIWInstruction=0; oneVLIWInstruction<sizeNewlyTranslated; oneVLIWInstruction++){
		...
		std::cerr << printDecodedInstr(vliwInstr.slc<32>(0));fprintf(stderr," ");
		std::cerr << printDecodedInstr(vliwInstr.slc<32>(32));fprintf(stderr," ");
		std::cerr << printDecodedInstr(vliwInstr.slc<32>(64));fprintf(stderr," ");
		std::cerr << printDecodedInstr(vliwInstr.slc<32>(96));fprintf(stderr," ");
		fprintf(stderr,"||\n");
