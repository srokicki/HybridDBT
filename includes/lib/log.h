...
public:
  ...
  static void printStat(DBTPlateform* platform, IRApplication* application)
  {
    ...
    if (_stat_mode == 1) {
      ...
      Log::logError << platform->vexSimulator->cycle << ";" << platform->vexSimulator->nbInstr << ";"
      ...
      Log::logError << "\t\t Average block size BEFORE trace/unrolling: " << platform->blockProcAverageSizeBeforeTrace
                    << " (M2 " << platform->blockProcDistanceBeforeTrace / (platform->nbBlockProcedureBeforeTrace - 1)
                    << ")\n";
      Log::logError << "\t\t Average block size AFTER trace/unrolling: " << platform->blockProcAverageSize << " (M2 "
                    << platform->blockProcDistance / (platform->nbBlockProcedure - 1) << ")\n";
