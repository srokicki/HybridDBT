...
public:
   ...
   */
  template <class... Ts> static void printf(char verbose, const char* format, Ts... args)
  {
   ...
   */
  template <class... Ts> static void fprintf(char verbose, FILE* f, const char* format, Ts... args)
  {
  ...
  static void printStat(DBTPlateform* platform, IRApplication* application)
  {
    ...
    if (_stat_mode == 1) {
      ...
      for (int oneConfig = 0; oneConfig < 32; oneConfig++) {
        ...
        energyConsumption +=
            platform->vexSimulator->timeInConfig[oneConfig] * period * getPowerConsumption(oneConfig) / 1000;
      }

      Log::fprintf(0, stdout, "%ld;%ld;%f;%d;%d;", platform->vexSimulator->cycle, platform->vexSimulator->nbInstr,
      ...
      for (int oneConfig = 0; oneConfig < 32; oneConfig++) {
        ...
        energyConsumption +=
            (platform->vexSimulator->timeInConfig[oneConfig] * period * getPowerConsumption(oneConfig));
      }

      Log::fprintf(0, stdout,
                   "Execution is finished...\nStatistics on the execution:\n\t Number of cycles: %ld\n\t Number of "
                   "instruction executed: %ld\n\t Average IPC: %f\n",
                   platform->vexSimulator->cycle, platform->vexSimulator->nbInstr,
                   ((double)platform->vexSimulator->nbInstr) / ((double)platform->vexSimulator->cycle));
      ...
      Log::fprintf(0, stdout, "\t\t Average block size BEFORE trace/unrolling: %f (M2 %f)\n",
                   platform->blockProcAverageSizeBeforeTrace,
                   platform->blockProcDistanceBeforeTrace / (platform->nbBlockProcedureBeforeTrace - 1));
...
public:
  template <class T> const LogStream& operator<<(const T& v)
  {
