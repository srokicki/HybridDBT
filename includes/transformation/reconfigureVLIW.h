 ...
 *      - Next one bit correspond to the extra memory slot: (0) for a config with only one mem, (1) for a config with
 *two mem
 *      - Next two bits corresponds to the extra mult slot: (00) for 1 mult, (01) for two, (10) for three
 *      - Last bit says if the configuration is boosted with basic instruction execution: each configuration of
 *(mem/mult) can be reached in two different issue widths. When this bit is at zero, it correspond to the smallest one,
